...
def _prepare_sys_path():
    """Add the project root to the system path to allow local imports."""
    if str(PROJECT_ROOT) not in sys.path:
        sys.path.insert(0, str(PROJECT_ROOT))


def get_config_classes() -> Dict[str, Type]:
    """Dynamically finds all configuration dataclasses."""
    _prepare_sys_path()
    config_classes = {}
    section_dir = SCALER_ROOT / "config" / "section"
    for path in section_dir.glob("*.py"):
        if path.name.startswith("_"):
            continue
        module_name = f"scaler.config.section.{path.stem}"
        try:
            module = importlib.import_module(module_name)
            for name, obj in inspect.getmembers(module, inspect.isclass):
                if dataclasses.is_dataclass(obj) and obj.__module__ == module_name:
                    config_classes[path.stem] = obj
        except ImportError as e:
            print(f"Warning: Could not import module {module_name}. Skipping. Error: {e}")
    return config_classes


def get_type_repr(type_hint: Any) -> str:
    """Creates a user-friendly string representation of a type hint."""
    if inspect.isclass(type_hint) and hasattr(type_hint, "from_string"):
        return "str"
