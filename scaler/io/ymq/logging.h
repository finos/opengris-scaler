...
class Logger {
public:
    enum LoggingLevel { critical, error, warning, info, debug, notset };

// Sound default logging level based on build type.
#ifdef NDEBUG  // Release build
    static constexpr LoggingLevel LOGGING_LEVEL = LoggingLevel::info;
#else  // Debug build
    static constexpr LoggingLevel LOGGING_LEVEL = LoggingLevel::debug;
#endif

    Logger(
        std::string log_format = "%(levelname)s: %(message)s",
        std::string log_path   = "/dev/stdout",
        LoggingLevel level     = LOGGING_LEVEL)
        : log_path_(std::move(log_path)), level_(level)
    {
    ...
    void log(LoggingLevel level, Args&&... args) const
    {
        ...
        for (const auto& part: M_processed_format) {
            if (part.is_token) {
        ...
        if (log_path_.empty() || log_path_ == "/dev/stdout") {
            ...
            if (log_file.is_open()) {
                ...
                throw std::runtime_error("Error: Could not open log file: " + log_path_ + "\n" + formatted_message);
            }
        }
    }
...
private:
    ...
    std::vector<FormatPart> M_processed_format;
    ...
    void preprocessFormat(const std::string& format)
    {
        ...
        while ((find_pos = format.find('%', last_pos)) != std::string::npos) {
            if (find_pos > last_pos) {
                M_processed_format.push_back({format.substr(last_pos, find_pos - last_pos), false});
            }

            if (format.length() > find_pos + 1 && format[find_pos + 1] == '%') {
                M_processed_format.push_back({"%", false});
                last_pos = find_pos + 2;
                continue;
            }
            ...
            if (format.length() > find_pos + 1 && format[find_pos + 1] == '(') {
                ...
                if (token_end != std::string::npos) {
                    ...
                    M_processed_format.push_back({token, true});
        ...
        if (last_pos < format.length()) {
            M_processed_format.push_back({format.substr(last_pos), false});
        }
    }
};
