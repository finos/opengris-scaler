...
void TcpClient::retry()
{
    if (_retryTimes > _maxRetryTimes) {
        logger_.log(Logger::LoggingLevel::error, "Retried times has reached maximum", _maxRetryTimes);
        exit(1);
        return;
    }

    logger_.log(Logger::LoggingLevel::debug, "Client retrying times", _retryTimes);
    CloseAndZeroSocket(_connFd);
