...
protected:
    ...
    void SetUp() override
    {
        // Remove the log file to ensure a clean slate for the test.
        std::remove(log_filename.c_str());
        server     = std::make_unique<scaler::object_storage::ObjectStorageServer>();
        serverPort = std::to_string(getAvailableTCPPort());

        // Start the server and configure it to write logs to a file.
        serverThread = std::thread(
            [this] { server->run(SERVER_HOST, serverPort, "INFO", "%(levelname)s: %(message)s", log_filename); });
        server->waitUntilReady();
    }
