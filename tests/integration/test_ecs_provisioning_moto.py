...
def _desired_requests(count: int):
    """The exact per-capset request list the scheduler's scaling policy sends over the wire."""
...
class TestECSProvisioningControlPlane(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_scale_down_stops_ecs_tasks(self) -> None:
        ...
        await provisioner.set_desired_task_concurrency(_desired_requests(12))  # ceil(12/4) = 3
        ...
        await provisioner.set_desired_task_concurrency(_desired_requests(4))  # ceil(4/4) = 1
    ...
    async def test_terminate_stops_all_tasks(self) -> None:
        ...
        provisioner = ECSWorkerProvisioner(_make_ecs_config(self.env, max_task_concurrency=-1, ecs_task_cpu=4))
        await provisioner.set_desired_task_concurrency(_desired_requests(8))
        await async_wait_until(lambda: provisioner.active_unit_count() == 2, message="scale-up to 2 units")
