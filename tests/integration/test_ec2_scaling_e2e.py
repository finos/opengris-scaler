...
from tests.integration._ec2_backend import run_ec2_worker_manager, serve_directory_on_gateway
...
_WHEEL_DIR = os.environ.get("SCALER_IT_MANYLINUX_WHEEL_DIR", os.path.join(_REPO_ROOT, "dist_manylinux"))
...
def _manylinux_wheel() -> str:
    wheels = sorted(glob.glob(os.path.join(_WHEEL_DIR, "*manylinux*.whl")))
...
if not RUN_EC2_E2E:
    ...
    _SKIP_REASON = f"no manylinux wheel under {_WHEEL_DIR}; build one with scripts/build_cibuildwheel.sh"
...
class TestEC2ScalingE2E(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._wheel_server = serve_directory_on_gateway(_WHEEL_DIR, "0.0.0.0", wheel_port)
        self.addCleanup(self._wheel_server.shutdown)
        wheel_url = f"http://{gateway}:{wheel_port}/{os.path.basename(_WHEEL)}"

        self.harness = SchedulerHarness(gateway=gateway, enable_webgui=True)
