...
TestResult basicServerYmq(std::string address)
{
    ...
    RETURN_FAILURE_IF_FALSE(scaler::ymq::as_string(*result->payload) == "yi er san si wu liu");
...
TestResult serverReceivesEmptyMessages(std::string address)
{
    ...
    auto result = socket.recvMessage();
    RETURN_FAILURE_IF_FALSE(result.has_value());
    RETURN_FAILURE_IF_FALSE(scaler::ymq::as_string(*result->payload) == "");
...
TestResult multicastSubscriber(std::string address, Identity identity)
{
    ...
    RETURN_FAILURE_IF_FALSE(scaler::ymq::as_string(*msg->payload) == "valid multicast message");

    msg = socket.recvMessage();
    RETURN_FAILURE_IF_FALSE(msg.has_value());
    RETURN_FAILURE_IF_FALSE(scaler::ymq::as_string(*msg->payload) == "valid broadcast message");
...
TestResult multicastPublisher(std::string address, Identity identityPrefix, int nSubscribers)
{
    ...
    for (int i = 0; i < nSubscribers; ++i) {
        auto msg = socket.recvMessage();
        RETURN_FAILURE_IF_FALSE(msg.has_value());
        RETURN_FAILURE_IF_FALSE(scaler::ymq::as_string(*msg->payload) == "ready");
    }
...
TestResult clientCloseEstablishedConnectionClient(std::string address)
{
    ...
    auto result = socket.recvMessage();
    RETURN_FAILURE_IF_FALSE(result.has_value());
    RETURN_FAILURE_IF_FALSE(scaler::ymq::as_string(*result->payload) == "1");
...
TestResult clientCloseEstablishedConnectionServer(std::string address)
{
    ...
    auto result = socket.recvMessage();
    RETURN_FAILURE_IF_FALSE(result.has_value());
    RETURN_FAILURE_IF_FALSE(scaler::ymq::as_string(*result->payload) == "0");
