...
from scaler.config.config import (
    ClusterConfig,
    ObjectStorageConfig,
    SchedulerConfig,
    WorkerCapabilities,
...
class TestConfigValues(unittest.TestCase):
    """Tests for individual ConfigValue helper classes."""
    ...
    def test_worker_names_config_value(self):
        """Test the WorkerNames ConfigValue class."""
    ...
    def test_worker_capabilities_config_value(self):
        """Test the WorkerCapabilities ConfigValue class."""
...
"""
    ...
    def test_load_from_toml_only(self):
        ...
        config = SchedulerConfig.get_config(self.config_path, Namespace())
    ...
    def test_command_line_overrides_toml(self):
        ...
        config = SchedulerConfig.get_config(self.config_path, args)
    ...
    def test_non_existent_config_file_raises_error(self):
        ...
        with self.assertRaises(FileNotFoundError):
            SchedulerConfig.get_config(non_existent_path, Namespace())
    ...
    def test_unknown_field_in_toml_raises_error(self):
        ...
        with self.assertRaises(ValueError):
            SchedulerConfig.get_config(extra_field_path, Namespace())

    def test_optional_field_handling(self):
        """Test that an Optional field is None by default and can be set."""
        config_no_storage = SchedulerConfig.get_config(self.config_path, Namespace())
        self.assertIsNone(config_no_storage.storage_address)

        optional_toml_path = os.path.join(self.temp_dir.name, "optional.toml")
        with open(optional_toml_path, "w") as f:
            f.write('scheduler_address = "tcp://127.0.0.1:1234"\nstorage_address = "tcp://127.0.0.1:5678"')
        config_with_storage = SchedulerConfig.get_config(optional_toml_path, Namespace())
        self.assertIsInstance(config_with_storage.storage_address, ObjectStorageConfig)
        self.assertEqual(config_with_storage.storage_address.port, 5678)
    ...
    def test_invalid_enum_in_toml_raises_error(self):
        ...
        with self.assertRaises(ValueError):
            SchedulerConfig.get_config(bad_enum_path, Namespace())
    ...
    def test_list_to_tuple_coercion(self):
        ...
        config = SchedulerConfig.get_config(config_path=None, args=args)
...
"""
    ...
    def test_load_from_toml_only(self):
        ...
        config = ClusterConfig.get_config(self.config_path, Namespace())
    ...
    def test_command_line_overrides_toml(self):
        ...
        config = ClusterConfig.get_config(self.config_path, args)
