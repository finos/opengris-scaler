...
public:
    ...
    void bind(const char* sAddr, int port)
    {
        ...
        auto status = ::bind(this->_fd, (sockaddr*)&addr, sizeof(addr));
        if (status < 0)
    ...
    std::pair<Socket, sockaddr_in> accept(int flags = 0)
    {
        ...
        auto fd       = ::accept4(this->_fd, (sockaddr*)&peer_addr, &len, flags);
        if (fd < 0)
...
inline TestResult test(int timeout_secs, std::vector<std::function<TestResult()>> closures, bool delay_fst = false)
{
    ...
    pfds.push_back({.fd = timerfd._fd, .events = POLL_IN, .revents = 0});
    for (auto pipe: pipes)
        pfds.push_back({
            .fd      = pipe.first,
            .events  = POLL_IN,
