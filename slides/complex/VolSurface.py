"""
═══════════════════════════════════════════════════════════════════════════════
EXAMPLE 5: Implied Vol Surface Calibration → Local Vol → PDE Exotic Pricing
═══════════════════════════════════════════════════════════════════════════════

This is the standard production pipeline at every equity derivatives desk:

  1. Calibrate a parametric vol surface (SVI per expiry) to market option quotes
     — one heavy optimization per expiry pillar, fully parallelizable
  2. Validate surface for static arbitrage (calendar spread + butterfly)
     — requires ALL expiries to be calibrated (fan-in point)
  3. Extract Dupire local vol surface
     — requires ALL calibrated SVI fits (another fan-in point)
  4. Price exotic options using a backward PDE on the local vol surface
     — one Crank-Nicolson solve per trade, fully parallelizable (fan-out)
  5. Compute vega ladder (bump each expiry pillar's vol by 1pt, re-solve PDE)
     — for each trade: N_expiries PDE re-solves (fan-out within trade)
  6. Aggregate book Greeks for risk report

DAG TOPOLOGY — the "hourglass" pattern:

  load_market_quotes()
          │
  ┌───────┼───────┬───────┬───────┐
  ▼       ▼       ▼       ▼       ▼
 SVI     SVI     SVI     SVI    SVI        ← Level 2: N_expiry parallel nodes
 3M      6M      1Y      2Y     3Y         (each: nonlinear opt 2–5 min)
  └───────┴───────┴───────┴───────┘
              │ (fan-in: all expiries needed)
    validate_arbitrage_surface()            ← Level 3: 1 node (depends on ALL SVI fits)
              │
    extract_local_vol_surface()             ← Level 4: 1 node (Dupire, expensive)
              │
  ┌───────────┼──────────────────────┐
  ▼           ▼                      ▼
 PDE         PDE                    PDE     ← Level 5: M_trade parallel PDE solves
trade_001   trade_002   ...       trade_M   (each: Crank-Nicolson ~3–8 min)
  │           │                      │
  ▼           ▼                      ▼
vega_ladder vega_ladder   ...   vega_ladder ← Level 6: M×N_expiry re-solves
 trade_001   trade_002           trade_M    (each: N_expiry bumped PDE solves)
  │           │                      │
  └───────────┴──────────────────────┘
              │ (fan-in: all trades)
    aggregate_book_greeks()                 ← Level 7: 1 aggregation node

Serial:   5×3 (SVI) + 2 (LocalVol) + 200×5 (PDE) + 200×5×5 (vega) ≈ 36 hours
Parallel: 5 (SVI max) + 2 (LV) + 5 (PDE max) + 25 (vega max) ≈ 37 min on large cluster
Speedup:  ~58×
═══════════════════════════════════════════════════════════════════════════════
"""
...
class LocalVolSurface:
    ...
    def sigma(self, T: float, K: float) -> float:
        ...
        return float(self._interp([[T_clipped, K_clipped]]))
...
if __name__ == "__main__":
    ...
    print("Running Vol Surface + PDE Exotic Pricing Pipeline...")
    print("(Use n_trades=5, compute_vega=False for a quick local test)")

    greeks = vol_surface_and_pde_pipeline(
        n_trades=10,  # use 50 on Scaler
        expiries=[0.25, 0.5, 1.0, 2.0, 3.0],  # use 8 on Scaler
        spot=100.0,
        rate=0.04,
        seed=42,
        compute_vega=False,  # set True on Scaler for full vega ladder
    )

    print(f"\nPortfolio NPV   : ${greeks.total_npv:>12,.0f}")
    print(f"Portfolio Delta : {greeks.portfolio_delta:>12.4f}")
    print(f"Portfolio Gamma : {greeks.portfolio_gamma:>12.6f}")

    if not greeks.vega_by_expiry.empty:
        print("\nVega Ladder ($ per 1 vol point):")
        for exp, vega in greeks.vega_by_expiry.items():
            print(f"  {exp:.2f}Y: ${vega:>10,.0f}")

    print("\nTop 5 Positions by NPV:")
    print(greeks.risk_report.head(5).to_string(index=False))

    # Visualise the hourglass DAG:
    # vol_surface_and_pde_pipeline.to_graph(
    #     n_trades=10, expiries=[0.25, 0.5, 1.0, 2.0], compute_vega=True
    # ).to_dot().write_png("hourglass_dag.png")
