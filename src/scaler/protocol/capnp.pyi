...
class TaskResult(CapnpStruct):
    taskId: TaskID
    resultType: TaskResultType
    metadata: bytes
    results: Any

class TaskCancelConfirm(CapnpStruct):
    taskId: TaskID
    cancelConfirmType: TaskCancelConfirmType

class GraphTask(CapnpStruct):
    taskId: TaskID
    source: ClientID
    targets: Any
    graph: Any

class ClientHeartbeat(CapnpStruct):
    resource: Resource
    latencyUS: int

class ClientHeartbeatEcho(CapnpStruct):
    objectStorageAddress: ObjectStorageAddress

class WorkerHeartbeat(CapnpStruct):
    agent: Resource
    rssFree: int
    queueSize: int
    queuedTasks: int
...
class WorkerHeartbeatEcho(CapnpStruct):
    objectStorageAddress: ObjectStorageAddress

class WorkerManagerHeartbeat(CapnpStruct):
    maxTaskConcurrency: int
    capabilities: Any
    workerManagerID: bytes

class WorkerManagerHeartbeatEcho(CapnpStruct): ...
...
class WorkerManagerCommand(CapnpStruct):
    workerIDs: Any
    command: WorkerManagerCommandType
    capabilities: Any

class WorkerManagerCommandResponse(CapnpStruct):
    command: WorkerManagerCommandType
...
class ObjectInstruction(CapnpStruct):
...
class DisconnectRequest(CapnpStruct):
    worker: WorkerID

class DisconnectResponse(CapnpStruct):
    worker: WorkerID

class ClientDisconnect(CapnpStruct):
...
class ClientShutdownResponse(CapnpStruct):
    accepted: bool

class StateClient(CapnpStruct): ...
class StateObject(CapnpStruct): ...

class StateBalanceAdvice(CapnpStruct):
    workerId: WorkerID
    taskIds: Any

class StateScheduler(CapnpStruct):
    binder: BinderStatus
    scheduler: Resource
    rssFree: int
...
class StateWorker(CapnpStruct):
    workerId: WorkerID
    state: WorkerState
    capabilities: Any

class StateTask(CapnpStruct):
    taskId: TaskID
    functionName: bytes
    state: TaskState
    worker: WorkerID
...
class StateGraphTask(CapnpStruct):
    graphTaskId: TaskID
    taskId: TaskID
...
class ProcessorInitialized(CapnpStruct): ...

class InformationRequest(CapnpStruct):
    request: bytes

class InformationResponse(CapnpStruct):
    response: bytes
