...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "sudo /opt/opengris-scaler/bin/pip install numpy scipy pandas pargraph opengris-parfun",
      ...
      "echo 'OPENBLAS_NUM_THREADS=4' | sudo tee -a /etc/environment",
      "echo 'GOTO_NUM_THREADS=4' | sudo tee -a /etc/environment",
      "echo 'OMP_NUM_THREADS=4' | sudo tee -a /etc/environment",
      "echo 'MKL_NUM_THREADS=4' | sudo tee -a /etc/environment"
    ]
  }
}
