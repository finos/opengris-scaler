...
class VanillaScalingPolicy(ScalingPolicy):
    ...
    Scale-down is gated by a per-worker-manager cooldown: once the policy first wants to
    reduce a manager's desired worker count below its current managed count, it holds at
    the current count for a cooldown window before applying the reduction. Any observation
    that calls for scale-up resets that manager's cooldown. See ScaleDownCooldown.
    """
    ...
    def get_scaling_commands(
        ...
        desired = cooldown.apply(desired, current)
